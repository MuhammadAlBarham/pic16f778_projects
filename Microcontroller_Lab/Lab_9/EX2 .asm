...
RECEIVE
	...
	movf	RCREG,w           ; move the data to W-REG 
	movf	RCREG,w			  ; move the data to W-REG 	 
	movwf	counter			  ; Save the Data 	
						 
							  ; I check the character range by subtract the upper character from the range 
							  ; And I used the Carry Flag as an Indicator 	
							  ; Then I check the Carry flag if C = 1  not the correct range else the correct range 
 	sublw	0x43			
	btfsc	STATUS,C
	goto 	ONE					
	...
	FINISH		
	call  	look_up					; call the look_up table to take the binary value to show the number 
	banksel PORTB 					; In the Seven seg. display 
	movwf 	PORTB																									
