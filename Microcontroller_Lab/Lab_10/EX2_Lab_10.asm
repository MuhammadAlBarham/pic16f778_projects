...
Initial	
	...
	movlw		0x02
	movwf		Counter
	clrf        D_val		; Clear the Digital Value 
...
L1	btfss	TXSTA,TRMT
	...
	banksel	TXREG
	movf	Pressure_Result,w
	call 	D_A_Convert			;Convert the vlaue after sent it 
...
L2	btfss	TXSTA,TRMT
	...
	call 	D_A_Convert               ;call the Digital to Anloge converter 
...
repair                     ;This subroutine return the positive value of number 
comf 	Difference,1	   ; By complement value,then incrmeant by 1 
incf	Difference,0
 return 
;***********************************************************
D_A_Convert			   ; This Subroutine Code convert the Value from Digital 
movwf 	D_val		   ; to analoge value 
movf 	D_val,0 	   ;Check the Value of D_val that will be sent 
sublw 	D'25'		   ;Then choose the best value according to the table 
btfsc	STATUS,C	   ;Then return the value to W_reg
retlw 	0x30

movf 	D_val,0		  ;choosen the max value in each range to ensure the over value in that range 
sublw 	D'76'		  ;subtract the value from the w-reg
btfsc	STATUS,C	  ;if yes return the corresbonding value acoording to the table 
retlw 	0x31		  ;else check anotheer range 
